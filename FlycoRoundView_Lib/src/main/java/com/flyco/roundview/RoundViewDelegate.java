...
public class RoundViewDelegate {
    ...
    private void obtainAttributes(Context context, AttributeSet attrs) {
        ...
        isDrawOutParent = ta.getBoolean(R.styleable.RoundTextView_rv_isDrawOutParent, true);
        ta.recycle();
    }
...
//              Log.d("AAA", textColors.getColorForState(new int[]{-android.R.attr.state_pressed}, -1) + "");
    ...
    private Path drawOutParentPath;
    
    private Paint drawOutParentPaint;
    
    public void drawOutParentBefore(Canvas canvas) {
        canvas.saveLayer(new RectF(0, 0, canvas.getWidth(), canvas.getHeight()),
                null, Canvas.ALL_SAVE_FLAG);
    }
    
    public void drawOutParentAfter(Canvas canvas) {
        ...
        if (drawOutParentPath == null) {
            drawOutParentPath = new Path();
        }
        drawOutParentPath.reset();
        //计算裁剪圆角
        ...
        if (drawOutParentPaint == null) {
            ...
            drawOutParentPaint.setAntiAlias(true);
            // 绘制模式为填充
            drawOutParentPaint.setStyle(Paint.Style.FILL);
    ...
    //是否绘制超过父布局
    private boolean isDrawOutParent = false;
    
    public boolean isDrawOutParent() {
        return isDrawOutParent;
    }
    
    public void setDrawOutParent(boolean drawOutParent) {
        isDrawOutParent = drawOutParent;
    }
